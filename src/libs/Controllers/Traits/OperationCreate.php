...
trait OperationCreate
{
    ...
    public function createmodal()
    {
        ...
        if ($this->tienePermiso('create')) {
            ...
            // load the view from /resources/views/vendor/backpack/crud/ if it exists, otherwise load the one in the package
            return view($this->getCreateModalView(), $this->filterCreateViewData($this->data));
        } else {
            return view('cesi::errors.401');
        }
    }
     ...
     *
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector|\Illuminate\Contracts\View\Factory|\Illuminate\View\View|\Illuminate\Http\JsonResponse
     */
        ...
        if ($this->tienePermiso('create')) {
            ...
            if ($request->ajax()) {
                return response()->json([
                    'fail' => true,
                    'msg' => trans('cesi::core.crud.insert_data_failed'),
                ]);
            } else {
