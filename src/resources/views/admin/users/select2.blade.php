...
$model_usr = new \Cesi\Core\app\Models\CesiUser();

$dflt_value = old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : false ));
$dflt_display = trans('cesi::core.users.select');
$item = null;

if($dflt_value) {
    $item = $model_usr->find($dflt_value);
    if ($item) {
...
@push('jquery_document_ready')
    var $select_menus = jQuery('#{{ $field['name'] }}').select2({
        ...
        allowClear: true,
        ajax: {
            url: '{{ route('admin.core.user.getdataajax') }}',
            dataType: 'json',
            delay: 250,
